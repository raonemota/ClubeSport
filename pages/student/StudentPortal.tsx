...
interface DailyClassGroupProps {
    date: string; // yyyy-MM-dd
    modalityId: string;
    sessions: ClassSession[];
}

const DailyClassGroup: React.FC<DailyClassGroupProps> = ({ date, modalityId, sessions }) => {
    const { modalities, getSessionBookingsCount, bookSession, bookings, currentUser, bookingReleaseHour } = useStore();
    const modality = modalities.find(m => m.id === modalityId);
    
    // Parse date for display
    const dateObj = new Date(sessions[0].startTime); // Take first session to get full date obj
    ...
    // Logic for Booking Release Rule
    let isLocked = false;
    let lockReason = "";

    // Rule:
    // Today: Always Open.
    // Tomorrow: Open only if current hour >= bookingReleaseHour.
...
// Original SessionCard kept for "My Bookings" view
...
const MyBookingCard: React.FC<SessionCardProps> = ({ session, bookingId, bookedAt, onRequestCancel }) => {
...
export const StudentPortal = () => {
  ...
  const filteredSessions = sessions
    ...
    .filter(s => {
        ...
        // 1. Hide sessions that are in the past (only if they are TODAY)
        if (isToday(sessionDate) && isPast(sessionDate)) {
             return false;
        }
        
        // 2. Hide sessions that are totally in the past (yesterday)
        if (isPast(sessionDate) && !isToday(sessionDate)) {
            return false;
        }

        // 3. Strict Rule: Show ONLY Today (0 diff) and Tomorrow (1 diff)
        return differenceInCalendarDays(sessionDate, now) <= 1 && differenceInCalendarDays(sessionDate, now) >= 0;
    })
    .sort((a,b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());

  // Group sessions for the "Browse" view
  // Key: "yyyy-MM-dd_modalityId"
  const groupedSessions = filteredSessions.reduce<Record<string, GroupedSession>>((acc, session) => {
  ...
  const groupedSessionList: GroupedSession[] = (Object.values(groupedSessions) as GroupedSession[]).sort((a, b) => a.date.localeCompare(b.date));
